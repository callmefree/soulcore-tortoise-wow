...
HOST = os.environ.get('SOULCORE_HOST', '125.110.90.129')  # 强制 IPv4（域名双栈会走 IPv6 超时）
PORT = int(os.environ.get('SOULCORE_PORT', '56789'))
USER = os.environ.get('SOULCORE_USER', 'administrator')
PASS = os.environ.get('SOULCORE_SSH_PASS', '674968117')
...
def run_mysql(sql_path, db='tw_world'):
    ...
    try:
        ...
        cmd = "mysql -uroot -psoulcore2026 --default-character-set=utf8 %s < %s; RC=$?; rm -f %s; exit $RC" % (db, remote, remote)
